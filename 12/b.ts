...
const getStartAndFinishAndOthers = (map: string[][]) => {
  ...
  for (let y = 0; y !== map.length; y++) {
    for (let x = 0; x !== map[0].length; x++) {
      ...
      if (map[y][x] === 'a') others.push({ x, y });
    }
  }

  return { start, finish, others };
};
...
const { start, finish, others } = getStartAndFinishAndOthers(mapInput);

const distances = [dijkstra(start, finish, mapInput)];

others.forEach((other) => {
  distances.push(dijkstra(other, finish, mapInput));
});
